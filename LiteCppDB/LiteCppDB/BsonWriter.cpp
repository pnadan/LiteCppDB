...
namespace LiteCppDB
{
	...
	{
		const auto count = doc.GetBytesCount(true);
	...
	void BsonWriter::WriteDocument(ByteWriter& writer, BsonDocument doc)
	{
		...
		writer.Write(gsl::narrow_cast<uint8_t>(0x00));
	}
	...
	void BsonWriter::WriteElement(ByteWriter& writer, std::string key, BsonValue value)
	{
		...
		{
			case BsonType::Null:
				writer.Write(gsl::narrow_cast<uint8_t>(0x0A));
				this->WriteCString(writer, key);
				break;
			case BsonType::Int32:
				writer.Write(gsl::narrow_cast<uint8_t>(0x10));
				this->WriteCString(writer, key);
				writer.Write(value.AsInt32());
				break;
			case BsonType::Double:
				writer.Write(gsl::narrow_cast<uint8_t>(0x13));
				this->WriteCString(writer, key);
				writer.Write(value.AsDouble());
				break;
			case BsonType::String:
				writer.Write(gsl::narrow_cast<uint8_t>(0x02));
				this->WriteCString(writer, key);
				this->WriteString(writer, value.AsString());
				break;
			case BsonType::MinValue:
				writer.Write(gsl::narrow_cast<uint8_t>(0xFF));
				this->WriteCString(writer, key);
				break;
			case BsonType::MaxValue:
				writer.Write(gsl::narrow_cast<uint8_t>(0x7F));
				this->WriteCString(writer, key);
				break;
			case BsonType::DateTime:
				writer.Write(gsl::narrow_cast<uint8_t>(0x09));
				this->WriteCString(writer, key);
	...
	void BsonWriter::WriteString(ByteWriter& writer, std::string s)
	{
		...
		writer.Write(gsl::narrow_cast<int32_t>(myVector.size()) + 1);
