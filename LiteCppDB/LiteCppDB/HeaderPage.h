...
namespace LiteCppDB
{
	...
	public:
		...
		uint16_t getChangeID();
		void setChangeID(uint16_t changeID);
		...
		uint32_t getFreeEmptyPageID();
		void getFreeEmptyPageID(uint32_t freeEmptyPageID);

		// Last created page - Used when there is no free page inside file
		uint32_t getLastPageID();
		void setLastPageID(uint32_t lastPageID);

		// Database user version [2 bytes]
		uint16_t getUserVersion();
		void setUserVersion(uint16_t UserVersion);

		// Password hash in SHA1 [20 bytes]
		std::array<uint8_t, 20> getPassword();
		void setPassword(std::array<uint8_t, 20> password);

		// When using encryption, store salt for password
		std::array<uint8_t, 16> getSalt();
		void setSalt(std::array<uint8_t, 16> salt);
...
#pragma region Read / Write pages
	...
	protected:
		void ReadContent(ByteReader reader);

		void WriteContent(ByteWriter writer);
