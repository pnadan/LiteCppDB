...
namespace LiteCppDB
{
	...
	PageType CollectionPage::getPageType()
	{
	...
	uint32_t CollectionPage::getFreeDataPageID()
	{
		return this->mFreeDataPageID;
	}

	/// Get the number of documents inside this collection
	int64_t CollectionPage::getDocumentCount()
	{
		return this->mDocumentCount;
	}
	void CollectionPage::setDocumentCount(int64_t documentCount)
	{
	...
	CollectionPage::CollectionPage(uint32_t pageID)
	{
		...
		for (auto i = 0; i < static_cast<int>(this->getIndexes(false).size()); i++)
		{
	...
	void CollectionPage::UpdateItemCount()
	{
		this->setItemCount(1); // fixed for CollectionPage
		this->setFreeBytes(0); // no free bytes on collection-page - only one collection per page
	}
...
#pragma region Read / Write pages
	...
	void CollectionPage::ReadContent(ByteReader reader)
	{
	...
	void CollectionPage::WriteContent(ByteWriter writer)
	{
...
#pragma region Methods to work with index array
	...
	{
		for (int32_t i = 0; i < static_cast<int32_t>(this->mIndexes.size()); i++)
		{
